...
if [ "$TRAVIS_REPO_SLUG" != "anitab-org/bridge-in-tech-backend" ]; then
...
# Get the latest code
cd $HOME
git clone --branch=$TRAVIS_BRANCH https://github.com/anitab-org/bridge-in-tech-backend.git
cd bridge-in-tech-backend
